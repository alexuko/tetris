...
class Piece {
    constructor(tetrominoe, color) {
        ...
        this.x = 4; //position X in the canvas
    ...
    rotate = (clockwise = true) => {
        ...
        // position 0 + 1 = 1 % 4 = 1
        // position 1 + 1 = 2 % 4 = 2
        this.position =  clockwise ? (this.position + 1) % this.tetrominoe.length
                                   : ((this.position - 1) + this.tetrominoe.length) % this.tetrominoe.length;
        //modulo negative in JS dows
    ...
    collision = (dir) => {
        for (let r = 0; r < this.activeTetrominoe.length; r++) { //loop through all of the rows
            for (let c = 0; c < this.activeTetrominoe[r].length; c++) { //for each row loop through all of the columns
                ...
                if (dir[0] === 'down') { 
                    // if Direction is going DOWN then check "Y" limits
                    if (new_y >= ROW) return true; //reached the limit, is has collided
                    if (new_y <= ROW) continue; // current square from tetrominoe is ok. So check NEXT
                } else { 
                    // if Direction is either LEFT or RIGHT then check "X" limits
...
let piece = new Piece(tetrominoes[0], colors[0])
